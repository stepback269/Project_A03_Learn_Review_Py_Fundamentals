#print(f'Date: 12/28_A3/2025   current editing line = search for "NEW"  =fatten() function')
...
#NEW
def um(n=1, _ = '~ ~ ~ ~ ~ ~ ~ ~ . . ......'):
    return _*n  #returns anonymous var repeated n times -use in speak actions

def Talk_go_(msage0= '~ ~ ~ ~ ~ ~ ~ ~', msage1= 'PYTTSx3 is talking here', msage2= 'hello', rate_ = 135, gender = 1):
    engine = talk.init()
...
def outSpeak_centrd_mssgs(mssg_type= 'speak_intro_', frame_id= '00', lo_hi_range= 'A-Z'):
    mssg_id_prefix = mssg_type + frame_id
    #--^^^-- the spoken message name is a concat of prefix plus an alpha char
    ...
    while mssg_id_i < hi_ascii + 1:     ### coding NOTE: put mssg_id_txt inside an exec() code string !!!!!
        ...
        Talk_go_(um(), gotten_mssg, um(), 127, 1)
        time.sleep(t2)
    return
