#print(f'Date: 12/18_A3/2025   copied from VERSION B** current editing line = search for "NEW"  = get_keystroke() function')
...
print(f'Talk has successfully been imported into funcs_02\n')

import pygame as pg
#pg.init()
#pg.mixer.init()
print(f'Pygame has successfully been imported into func_02\n')
...
def qab_cwd():      #query about (what is) the Current Working Directory
    cwd = str(os.getcwd())
    print(f'Current working directory is {os.getcwd()}')  #-- https://www.youtube.com/watch?v=VfRuZOFNfpc
    return cwd

target_save_path1 = f'C:\\Users\\gideon\\Python_Projects_A0\\Project_A03_Learn_Fundamentals\\.venv\\Package_03\\'
target_save_path_ext2 = f'waves_01\\'

def qif_cwd_is_target(target_save_path1, target_save_path_ext2):      #query if CWD equals target
    cwd = str(os.getcwd())
    total_path =target_save_path1 + target_save_path_ext2
    return cwd == total_path  #-- Returns True if current equals target

def qif_Talk_ready():
    engine = talk.init()
    print(f'Talk has successfully initialized')
    voices = engine.getProperty('voices')
    print(f'The voices of Talk are as follows:\n{voices}')
    return

def Talk_go(msage0= '__', msage1= 'PYTTSx3 is talking here. Testing one too three four five', msage2= 'hello', rate_ = 135, gender = 1):
    engine = talk.init()
    voices = engine.getProperty('voices')
...
def Talk_save(target_save_path1, target_save_path_ext2, message= 'PYTTSx3 wrote this here'):
    engine.setProperty('voice', voices[1].id)
    engine.setProperty('rate', 150)
    target_file = str(target_save_path1) + str(target_save_path_ext2)
    engine.save_to_file(message, target_file)
