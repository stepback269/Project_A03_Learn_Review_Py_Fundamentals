...
... more to follow
'''
from Package_03 import vars_01 as v #-- we are NOT importing the primary vars because main already loaded them !!!
...
intro_00b = f'{v.yy_}(Current version includes working with LISTS and MODULES){v.W_}'
speak_intro_00b = f'Welcome to the introduction pane of L L M version A two'

intro_00A = f'(00)  <-- This is the {v.yy_}current lesson frame identifier (alphanumeric){v.W_}'
intro_00B = f'The frame number (display_id) {v.yy_}will normally be incrementing{v.W_} as we advance thru lessons'
...
speak_intro_00A = f'At the top left you see the current lesson frame identifier as zero zero'
speak_intro_00B = f'The frame number  or display idea will normally be incrementing as we advance through lessons'
speak_intro_00C = f'You can advance by hitting the c or Space keys {fn.um(4)} Use the em key to expose aside lessons'
speak_intro_00D = f'We will be exercising various Python features {fn.um()} for example List Methods'
speak_intro_00E = (f'Click the blue link to turn on study music from a pre set U R L {fn.um()} Many Python features are '
                   f'brought to light {fn.um()},  including but not limited to the following')
...
speak_intro_00F = f'At the top left you see the current lesson frame identifier as zero zero'
speak_intro_00G = f'The frame number  or display idea will normally be incrementing as we advance through lessons'
speak_intro_00H = f'You can advance by hitting the c or Space keys {fn.um(4)} Use the em key to expose aside lessons'
speak_intro_00I = f'We will be exercising various Python features {fn.um()} for example List Methods'
speak_intro_00J = (f'Click the blue link to turn on study music from a pre set U R L {fn.um()} Many Python features are '
                   f'brought to light {fn.um()},  including but not limited to the following')
speak_intro_00K = f'At the top left you see the current lesson frame identifier as zero zero'
speak_intro_00L = f'The frame number  or display idea will normally be incrementing as we advance through lessons'
speak_intro_00M = f'You can advance by hitting the c or Space keys {fn.um(4)} Use the em key to expose aside lessons'
speak_intro_00N = f'At the top left you see the current lesson frame identifier as zero zero'
speak_intro_00O = f'The frame number  or display idea will normally be incrementing as we advance through lessons'
speak_intro_00P = f'You can advance by hitting the c or Space keys {fn.um(4)} Use the em key to expose aside lessons'
